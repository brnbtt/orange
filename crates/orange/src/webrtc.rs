...
mod tests {
    ...
    fn audio_control_receive_worker_cancels_while_overlay_is_locked() {
        if run_in_bounded_subprocess(
            ...
            "webrtc::tests::audio_control_receive_worker_cancels_while_overlay_is_locked",
        ) {
            return;
        }
        gst::init().unwrap();
    ...
    fn incoming_bitrate_receive_worker_cancels_while_overlay_is_locked() {
        if run_in_bounded_subprocess(
            ...
            "webrtc::tests::incoming_bitrate_receive_worker_cancels_while_overlay_is_locked",
        ) {
            return;
        }
        gst::init().unwrap();
    ...
    fn incoming_bitrate_receive_worker_removes_probe_on_shutdown() {
        if run_in_bounded_subprocess(
            ...
            "webrtc::tests::incoming_bitrate_receive_worker_removes_probe_on_shutdown",
        ) {
            return;
        }
        gst::init().unwrap();
    ...
    fn receive_pad_acceptance_keeps_first_audio_and_video_owners() {
        if run_in_bounded_subprocess(
            ...
            "webrtc::tests::receive_pad_acceptance_keeps_first_audio_and_video_owners",
        ) {
            return;
        }
        gst::init().unwrap();
    ...
    fn receive_worker_registry_close_waits_for_active_pad_callback() {
        if run_in_bounded_subprocess(
            ...
            "webrtc::tests::receive_worker_registry_close_waits_for_active_pad_callback",
        ) {
            return;
        }
        let registry = ReceiveWorkerRegistry::new();
