...
function Test-BetaManifest {
    ...
    if ($Manifest.version -cnotmatch '^0\.2\.0-beta\.[0-9]+$') { throw "Invalid beta version" }
    ...
    if ($Manifest.notes -isnot [string] -or $Manifest.notes.Length -gt 500 -or $Manifest.notes -match '[\x00-\x08\x0B\x0C\x0E-\x1F]') {
...
function Invoke-BetaPublish {
    ...
    try {
        ...
        if ($Publish) {
            ...
            if ($LASTEXITCODE -ne 0) { throw "Publishing requires HEAD on origin/main" }
        }
        ...
        if ($version -cnotmatch '^0\.2\.0-beta\.[0-9]+$') { throw "Workspace version is not a beta version" }
        ...
        if ($Publish) {
            Invoke-Checked -Command "az" -Arguments @(
                ...
                "--name", $installerName, "--auth-mode", "key", "--overwrite", "true", "--only-show-errors"
            )
            Invoke-Checked -Command "az" -Arguments @(
                "storage", "blob", "upload", "--account-name", $script:BetaStorageAccount,
            ...
            $tag = "v$version"
            if (-not (Test-NativeSuccess -Command "gh" -Arguments @("release", "view", $tag, "--repo", $script:Repository))) {
            ...
            Invoke-Checked -Command "gh" -Arguments @(
                "release", "upload", $tag, $installer, $manifestPath,
                "--repo", $script:Repository, "--clobber"
            )
        }
