...
This file is for developers. End-user documentation lives in
[README.md](README.md). [ARCHITECTURE.md](ARCHITECTURE.md) is the
current-state authority on process boundaries, source maps, media flows,
contracts and validation; nothing here supersedes it.
...
`. .\dev.ps1` is required before any `cargo` or `gst-inspect-1.0` invocation.
...
Run the complete validation matrix from
[ARCHITECTURE.md#validation](ARCHITECTURE.md#validation).
...
rather than shipped as audio files. There is no mute control.
...
are handed the code without being told it. Someone not on the list
...
- Commit subjects are imperative and descriptive, no prefixes. Bodies explain
  the evidence and the reasoning, and say plainly what the change does *not* fix.
- Comments explain why, tied to the concrete thing that went wrong.
